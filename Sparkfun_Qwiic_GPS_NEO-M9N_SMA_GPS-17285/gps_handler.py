...
class GpsDashboardApp(tk.Tk):
    ...
    def _load_trip_history_to_tree(self):
        ...
        for trip in self.trip_history:
            ...
            #distance_str = f"{trip['distance']:.2f}" if not math.isnan(trip['distance']) else "N/A"
            #max_speed_str = f"{trip['max_speed']:.2f}" if not math.isnan(trip['max_speed']) else "N/A"
    ...
    def _create_driving_dashboard_widgets(self, parent):
        ...
        top_row_frame = ttk.Frame(parent, padding=5)
        ...
        coords_accuracy_frame = ttk.Frame(parent, padding=5)
    ...
    def _draw_compass_template(self):
        """Draws the static elements of the vehicle-style compass plot."""
        self.logger.log_debug("Drawing compass template.")
        colors = self._get_plot_colors()
        ax = self.ax_compass
        ...
        self.logger.log_debug("Compass template drawn.")
    ...
    def _create_settings_widgets(self, parent):
        ...
        gps_settings_frame.grid(row=0, column=0, sticky="ew", pady=5, padx=5)
        ...
        # Display current setting from settings_manager (read-only)
        ...
        # Adjusted grid row to account for removed GNSS frame
        port_cfg_frame.grid(row=1, column=0, padx=5, pady=5, sticky="ew")
        ...
        log_settings_frame.grid(row=0, column=1, sticky="ew", pady=5, padx=5) # Adjusted row/column
        ...
        # New setting for console output
        ...
        # New setting for console output to file
        ...
        # FIX: Corrected variable name from self.trip_log_var to self.trip_log_dir_var
        ...
        # Adjusted grid row to account for removed GNSS and Port Config frames
        theme_frame.grid(row=2, column=1, sticky="ew", pady=5, padx=5)
        ...
        # Adjusted grid row to account for removed GNSS and Port Config frames
        unit_preference_frame.grid(row=3, column=1, sticky="ew", pady=5, padx=5)
        ...
        offline_playback_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=5, padx=5)
        ...
        # Initialize buttons here so they can be passed to OfflinePlaybackManager
        ...
        # Adjusted grid row to account for new offline playback frame
        storage_info_frame.grid(row=5, column=0, columnspan=2, sticky="ew", pady=5, padx=5)
    ...
    def _handle_offline_mode_toggle(self):
        ...
        self._toggle_offline_mode_ui(is_offline)

        if is_offline:
            # If switching TO offline mode, stop live GPS
            ...
            # Start offline playback if a file is loaded
            if self.offline_playback_manager.loaded_data:
                # If there's loaded data, set buttons to ready state for playback
                self.offline_playback_manager.play_button.config(state=NORMAL)
                self.offline_playback_manager.pause_button.config(state=DISABLED)
                self.offline_playback_manager.stop_button.config(state=DISABLED)
            else:
                # No data loaded, all playback buttons disabled
                self.offline_playback_manager.playback_status_var.set("No file loaded")
                self.offline_playback_manager.play_button.config(state=DISABLED)
                self.offline_playback_manager.pause_button.config(state=DISABLED)
                self.offline_playback_manager.stop_button.config(state=DISABLED)
        else:
            # If switching FROM offline mode, stop playback
            self.offline_playback_manager.stop_playback() # This will handle button states for stopping
            # Restart live GPS thread
            self.stop_live_gps_event.clear()
            # Only start live GPS thread if UBLOX_GPS_AVAILABLE is True
            if UBLOX_GPS_AVAILABLE:
                ...
                # Reset live GPS connection status
    ...
    def _toggle_offline_mode_ui(self, is_offline_mode):
        ...
        # Offline playback related controls
        ...
        # Disable play/pause/stop if no file is loaded, even if offline mode is active
        if is_offline_mode:
            if self.offline_playback_manager.loaded_data:
                # If data is loaded, enable play and disable pause/stop initially
                self.play_button.config(state=NORMAL)
                self.pause_button.config(state=DISABLED)
                self.stop_button.config(state=DISABLED)
            else:
                # If no data loaded, disable all playback controls
                self.play_button.config(state=DISABLED)
                self.pause_button.config(state=DISABLED)
                self.stop_button.config(state=DISABLED)
        else:
            # If not in offline mode, disable all playback controls
            self.play_button.config(state=DISABLED)
            self.pause_button.config(state=DISABLED)
            self.stop_button.config(state=DISABLED)
    ...
    def _generate_folium_map(self):
        ...
        for gf in self.geofences:
            ...
            folium.Marker(
                location=[gf['latitude'], gf['longitude']],
                popup=f"<b>{gf['name']}</b><br>Lat: {gf['latitude']:.4f}<br>Lon: {gf['longitude']:.4f}<br>Radius: {gf['radius']:.2f}m",
    ...
    def _check_for_gps_data(self):
        ...
        try:
            while True:
                status_msg = self.logger.queue.get_nowait()
                self.status_label.config(text=status_msg, bootstyle="warning")
                self.logger.log_warning(f"Status message from DataLogger: {status_msg}")
        except queue.Empty:
            pass
    ...
    def _process_gps_data(self, data):
        ...
        if "status" in data:
            ...
            self.logger.log_info(f"GPS Data Handler Status: {data['status']}")
        ...
        if self.is_trip_active:
            ...
            # A simple heuristic: if speed is very large (e.g., < 100 m/s), assume it's already m/s.
            # Otherwise, assume it's mm/s and convert.
            # A more robust solution would involve a flag in the data or knowing the source.
            if speed_mmps_raw > 1000: # Arbitrary threshold, assuming typical speeds won't exceed 1000 m/s (3600 km/h)
    ...
    def _update_compass(self, heading):
        ...
        ax.plot([0], [0.9], marker='^', markersizes=12, color='red', zorder=10)

        # Fixed Bottom Indicator (Small Needle Below - Blue Triangle)
        ax.plot([0], [0.1], marker='v', markersizes=8, color='blue', zorder=10)
